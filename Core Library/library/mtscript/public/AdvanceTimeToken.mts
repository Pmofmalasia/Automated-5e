...
[h,foreach(tempToken,AdvanceTimeTokens),CODE:{
	...
	[h,foreach(condition,OldConditionGroups),CODE:{
		...
		[h:setProperty("a5e.stat.ConditionList",json.path.setcarefully(getProperty("a5e.stat.ConditionList"),"[*][?(@.GroupID=="+json.get(condition,"GroupID")+")]['Duration']",newDuration))]
	}]
	...
	[h,foreach(item,ItemsWtihConditions),CODE:{
		...
		[h,foreach(tempItemCondition,itemConditionsWithDuration): UpdatedItem = json.path.setcarefully(UpdatedItem,"['ItemConditions'][?(@.GroupID =='"+json.get(tempItemCondition,"GroupID")+"')]['Duration']",pm.a5e.AdvanceTime(json.set(TimeAdvancedData,"Duration",json.get(tempItemCondition,"Duration"))))]

		[h:setProperty("a5e.stat.Inventory",json.path.setcarefully(getProperty("a5e.stat.Inventory"),"[*][?(@.ItemID == '"+json.get(UpdatedItem,"ItemID")+"')]",UpdatedItem))]		
	...
	[h,foreach(item,PerishableInventory),CODE:{
		...
		[h,if(ExpiredTest):
			setProperty("a5e.stat.Inventory",json.path.deletecarefully(getProperty("a5e.stat.Inventory"),"[*][?(@.ItemID=="+json.get(item,"ItemID")+")]"));
			setProperty("a5e.stat.Inventory",json.path.setcarefully(getProperty("a5e.stat.Inventory"),"[*][?(@.ItemID=="+json.get(item,"ItemID")+")]['Duration']",newDuration))
		]
	...
	[h,foreach(ability,validAbilities),CODE:{
		...
		[h:setProperty("a5e.stat.AllFeatures",json.path.setcarefully(getProperty("a5e.stat.AllFeatures"),"[*][?("+pm.a5e.PathFeatureFilter(ability)+")]['Duration']",newDuration))]
	}]
