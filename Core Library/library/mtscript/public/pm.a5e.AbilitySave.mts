...
[h,if(DCType=="UNKNOWN"),CODE:{
	...
	[h,if(isNumber(DCType)),CODE:{
		[h:pm.DC = pm.DCInfo]
		[h:pm.DCPrimeStat = ""]
	};{
		[h:pm.DCPrimeStat = pm.DCInfo]
		[h:pm.DC = 8 + json.get(AtrMods,pm.DCPrimeStat)]
	}]
};{
	[h:pm.baseDC = json.get(pm.DCInfo,"Base")]
	[h:pm.DCPrimeStat = json.get(pm.DCInfo,"Stat")]
	[h:pm.DC = pm.baseDC + json.get(AtrMods,pm.DCPrimeStat)]
}]
...
[h:macro.return = json.set("","Table",json.set("","ShowIfCondensed",1,"Header","Saving Throw","FalseHeader","","FullContents",pm.SaveType,"RulesContents"," - <b>DC <span style='font-size:1.5em'>"+pm.DCFinal+"</span></b>","RollContents","","DisplayOrder","['Full','Rules','Roll']"),"SaveDC",pm.DCFinal,"SaveType",pm.SaveType,"SaveData",saveDataFinal)]