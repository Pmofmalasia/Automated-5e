...
[h,if(pm.FromCharacter == 1),CODE:{
	...
	[h:pm.AbilitiesList = "[]"]
	[h,foreach(ability,pm.TempAbilitiesList): pm.AbilitiesList = json.merge(pm.AbilitiesList,json.path.read(data.getData("addon:","pm.a5e.core","sb.Abilities"),"\$[*][?(@.Name=='"+json.get(ability,"Name")+"' && @.Class=='"+json.get(ability,"Class")+"' && @.Subclass=='"+json.get(ability,"Subclass")+"' && @.Master.Name=='"+json.get(json.get(ability,"Master"),"Name")+"' && @.Master.Class=='"+json.get(json.get(ability,"Master"),"Class")+"' && @.Master.Subclass=='"+json.get(json.get(ability,"Master"),"Subclass")+"')]"))]
};{
	[h:pm.AbilitiesList = json.path.read(data.getData("addon:","pm.a5e.core","sb.Abilities"),"\$[*][?(@.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"' && @.Level<="+currentFeatureLevel+")]")]
...
[h,if(IsTooltip),CODE:{
	[h:pm.AbilitiesChosenStr = json.toList(json.path.read(getProperty("a5e.stat.AllFeatures"),"\$[*][?(@.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"' && @.IsActive > 0)]['DisplayName']"),if(json.get(data.getData("addon:","pm.a5e.core","TooltipVertical"),getPlayerName())=="",if(json.get(data.getData("addon:","pm.a5e.core","TooltipVertical"),"Default")==1,"<br>",", "),if(json.get(data.getData("addon:","pm.a5e.core","TooltipVertical"),getPlayerName())==1,"<br>",", ")))]
};{
