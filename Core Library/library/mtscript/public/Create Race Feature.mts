...
[h:abort(input(
	...
	" ab.Class | "+ab.RaceList+" | Associated Race | LIST | VALUE=STRING ",
...
[h:ab.Final = json.set("",
	...
	"Type","Race",
	"Class",js.a5e.RemoveSpecial(ab.Class),
	"Level",ab.Level,
...
[h:ab.Subclass="None"]
[h:abort(input(
	" ab.Subclass | None,"+pm.GetSubraces(ab.Class,"DisplayName")+" | Select associated subrace | LIST | VALUE=STRING ",
	if(ab.HasMaster," ab.MasterCreated | No,Yes,Yes - Same Race/Subrace Combo | Is the master feature already added to the core library? | RADIO | SELECT=2",""),
	if(ab.Replace," ab.ReplaceCreated | No,Yes,Yes - Same Race/Subrace Combo | Is the feature you would like to replace already added to the core library? | RADIO | SELECT=2","")
))]

[h:ab.Subclass = if(ab.Subclass == "None","",js.a5e.RemoveSpecial(ab.Subclass))]
[h:ab.Final = json.set(ab.Final,"Subclass",ab.Subclass)]

[h,if(ab.HasMaster),CODE:{
	[h,if(ab.MasterCreated>0),CODE:{
		[h,if(ab.MasterCreated==2): ab.MasterInput=""; ab.MasterInput=" ab.MasterClass | "+ab.RaceList+" | Race associated with Master Feature | LIST | VALUE=STRING ## junkVar | Next Screen | Subrace Selection | LABEL"]
		[h:abort(input(ab.MasterInput))]
		
		[h,if(ab.MasterCreated==2): ab.MasterInput=""; ab.MasterInput=" ab.MasterSubclass | None,"+pm.GetSubraces(ab.MasterClass,"DisplayName")+" | Subrace associated with Master Feature | LIST | VALUE=STRING "]
		[h:abort(input(ab.MasterInput))]

		[h,if(ab.MasterCreated==2): ab.MasterClass = ab.Class]
		[h,if(ab.MasterCreated==2): ab.MasterSubclass = ab.Subclass; ab.MasterSubclass = js.a5e.RemoveSpecial(ab.MasterSubclass)]
		...
		[h:abort(input(
			...
			" ab.MasterClass | "+ab.RaceList+" | Race associated with Master Feature | LIST | VALUE=STRING ",
		" junkVar | Next Screen | Subrace Selection | LABEL "
		))]
			
		[h:ab.MasterSubclass="None"]
		[h:abort(input(
			" ab.MasterSubclass | None,"+pm.GetSubraces(ab.Class,"DisplayName")+" | Subrace associated with Master Feature | LIST | VALUE=STRING "
			))]
...
[h,if(ab.Replace),CODE:{
	[h:abort(input(" ab.ReplaceCreated | No,Yes,Yes - Same Race/Subrace Combo | Is the feature you would like to replace already added to the core library? | RADIO | SELECT=2"))]
	[h,if(ab.ReplaceCreated>0),CODE:{
		[h,if(ab.ReplaceCreated==2): ab.ReplaceInput=""; ab.ReplaceInput=" ab.ReplaceClass | "+ab.RaceList+" | Choose an associated race | LIST | VALUE=STRING ## junkVar | Next Screen | Subrace Selection | LABEL"]
		[h:abort(input(ab.ReplaceInput))]
		
		[h,if(ab.ReplaceCreated==2): ab.ReplaceInput=""; ab.ReplaceInput=" ab.ReplaceSubclass | None,"+pm.GetSubraces(ab.Class,"DisplayName")+" | Select associated subrace | LIST | VALUE=STRING "]
		[h:abort(input(ab.ReplaceInput))]

		[h,if(ab.ReplaceCreated==2): ab.ReplaceClass = ab.Class]
		[h,if(ab.ReplaceCreated==2): ab.ReplaceSubclass = ab.Subclass; ab.ReplaceSubclass = js.a5e.RemoveSpecial(ab.ReplaceSubclass)]
		...
		[h:ab.ReplacedAbility = json.set("","Name",js.a5e.RemoveSpecial(ab.ReplaceName),"DisplayName",ab.ReplaceName,"Class",ab.ReplaceClass,"Subclass",if(ab.ReplaceSubclass=="None","",js.a5e.RemoveSpecial(ab.ReplaceSubclass)))]
	};{
		[h:abort(input(
		...
		[h:abort(input(
			" ab.ReplaceSubclass | None,"+pm.GetSubraces(ab.Class,"DisplayName")+" | Select associated subrace | LIST | VALUE=STRING "
			))]
...
[r:ab.DisplayName+" racial feature from the sourcebook "+ab.Source+" created."]
