...
[h,switch(ResourceType),CODE:
	...
	case "Hit Dice":{
		...
		[h,if(json.get(CheckResourceData,"Size")==""),CODE:{
			...
			[h:foreach(dieSize,AllDieSizes): RemainingResource = RemainingResource + number(json.get(CurrentHitDice,"1d"+dieSize))]
		};{
			[h:RemainingResource = number(json.get(CurrentHitDice,"1d"+json.get(CheckResourceData,"Size")))]
		}]
	};
	default:{
