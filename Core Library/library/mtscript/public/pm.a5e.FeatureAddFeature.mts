...
[h:pm.CurrentAbilitiesTemp = json.path.read(allAbilities,"[?(@.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"' && @.IsActive > 0)]")]
...
[h,if(pm.FromCharacter == 1),CODE:{
	[h:pm.TempAbilitiesList = json.path.read(allAbilities,"[?(@.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"')]")]
	[h:pm.AbilitiesList = "[]"]
...
[h,if(IsTooltip),CODE:{
	...
	[h,foreach(ability,pm.AbilitiesList),CODE:{
		[h:set("pm.Choose"+json.get(ability,"Name"),if(json.isEmpty(json.path.read(allAbilities,"[?(@.Name=='"+json.get(ability,"Name")+"' && @.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"' && @.Class=='"+json.get(ability,"Class")+"' && @.Subclass=='"+json.get(ability,"Subclass")+"' && @.IsActive > 0)]")),0,1))]
	...
	[h:pm.AbilitiesChosenStr = json.toList(json.path.read(allAbilities,"[?(@.Master.Name=='"+currentFeatureName+"' && @.Master.Class=='"+currentFeatureClass+"' && @.Master.Subclass=='"+currentFeatureSubclass+"' && @.IsActive > 0)]['DisplayName']"),if(getLibProperty("VerticalDisplay","Lib:pm.a5e.Core")==1,"<br>",", "))]
}]
