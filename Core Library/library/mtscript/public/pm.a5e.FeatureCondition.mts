...
[h,if(pm.IsToggle),CODE:{
	[h,SWITCH(IsTooltip+""+pm.ActiveConditionTest),CODE:
		...
		case "11":{
			[h:pm.CurrentDuration = pm.a5e.GenerateTimeDisplay(json.get(json.path.read(getProperty("a5e.stat.ConditionList"),"\$[*][?(@.GroupID=='"+json.get(pm.GroupIDTest,0)+"')]['Duration']"),0))]
	...
	[h,if(IsTooltip),CODE:{
		...
		[h,if(json.get(currentFeatureInfo,"isEffectResolving") == 1):
			buildEffectData = json.set("","CurrentEffects",json.append("",effFull),"ToMerge",effectsToMerge,"BaseEffect","{}","WhichEffect",0);
			buildEffectData = json.set("","CurrentEffects",pm.a5e.EffectData,"ToMerge",effectsToMerge,"BaseEffect",pm.a5e.BaseEffectData,"WhichEffect",whichEffect)
		]
		[h,MACRO("Build Effect@Lib:pm.a5e.Core"): buildEffectData]
		[h,if(json.get(currentFeatureInfo,"isEffectResolving") == 1):
			effFull = json.get(macro.return,0);
			pm.a5e.EffectData = macro.return
		]
	}]
}]