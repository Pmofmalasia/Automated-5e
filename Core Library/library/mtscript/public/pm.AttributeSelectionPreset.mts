...
[h,foreach(TempAtr,pm.GetAttributes("json")): ab.Input = listAppend(ab.Input," ab."+TempAtr+"Choice | -4,-3,-2,-1,0,1,2,3,4 | "+TempAtr+" | LIST | SELECT=4 VALUE=STRING ","##")]

[h:abort(input(ab.Input))]

[h:ab.AtrChoices = ""]
[h,if(ab.AllChoice!=0): ab.AtrChoices = json.set(ab.AtrChoices,"All",number(ab.AllChoice))]
[h,foreach(TempAtr,pm.GetAttributes("json")): ab.AtrChoices = if(number(eval("ab."+TempAtr+"Choice"))==0,ab.AtrChoices,json.set(ab.AtrChoices,TempAtr,number(eval("ab."+TempAtr+"Choice"))))]
