...
[h:NewInventory = json.path.setcarefully(CurrentInventory,"[*][?(@.isWearable == 1 || @.isAttunement == 1)]['IsActive']",0)]
[h:NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.isAttunement == 1)]['AttunedTo']","")]

[h:ArmorChoice = json.get(EquipItemData,"ArmorChoice")]
[h:setProperty("a5e.stat.EquippedArmor",ArmorChoice)]
[h,if(ArmorChoice!=""): NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+ArmorChoice+")]['IsActive']",1)]
...
[h,count(LimbNumber),CODE:{
	...
	[h,if(thisLimbChoice != ""): NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+thisLimbChoice+")]['IsActive']",1)]

	[h,if(json.contains(EquipItemData,"AmmunitionChoiceLimb"+roll.count)),CODE:{
		[h:NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+thisLimbChoice+")]['AmmunitionID']",json.get(EquipItemData,"AmmunitionChoiceLimb"+roll.count))]
...
[h,count(AttunementNumber),CODE:{
	...
	[h,if(thisAttunementChoice != ""),CODE:{
		...
		[h:NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+thisAttunementChoice+")]['AttunedTo']",ParentToken)]
		[h:NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+thisAttunementChoice+")]['IsActive']",1)]
	}]
}]
...
[h,foreach(wearableItem,AllWearables),CODE:{
	...
	[h,if(json.contains(EquipItemData,"WearableChoice"+thisItemID)): NewInventory = json.path.setcarefully(NewInventory,"[*][?(@.ItemID == "+thisItemID+")]['IsActive']",1)]
}]
