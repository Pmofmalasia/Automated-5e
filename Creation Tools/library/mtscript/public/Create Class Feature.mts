...
[h:abort(input(
	...
	" ab.Optional | 0 | <html><span title='For sourcebooks that add optional additional rules'>Optional Feature</span></html> | CHECK ",	" ab.Replace | 0 | <html><span title='For sourcebooks that optionally remove old features and replace them with new ones. Type the name of the replaced feature.'>Replaces another Feature if Gained</span></html> | CHECK ",		" junkVar | -------------------------------------------------------------------------------------------------------------- |  | LABEL | SPAN=TRUE ",	"ab.Prereqs |  | <html><span title='Mostly for features not gained automatically on level up, like Invocations or Feats. Ignore if level/class/subclass are the only prerequisites.'>Feature has prerequisites for being gained</span></html> | CHECK ",	"ab.MultiFeature |  | <html><span title='Pretty much just for Elemental Adept at this point.'>Feature can be gained multiple times</span></html> | CHECK "
	))]
